...
sequenceDiagram
    ...
    nuke ->> service: {service url}<br>?nuke_identifier={nuke identifier}<br>&nuke_token={nuke token}<br>&redirect_uri={nuke app url}
...
sequenceDiagram
    ...
    service ->> nuke: {nuke app url}<br>?nuke_identifier={nuke identifier}<br>&service_token={service token}<br>&redirect_uri={service url}
